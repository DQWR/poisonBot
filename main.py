...
markup = ReplyKeyboardMarkup(resize_keyboard=True).add(price_btn, contact_btn, order_btn, menu_btn)
...
async def process_start_command(message: types.Message):
    await message.answer('Привет! Это тестовый бот для заказа товаров. '
                         'Напишите /order, чтобы начать оформление заказа.', reply_markup=markup)
